...
export default function TorrentDetail({params}: { params: { slug: string } }) {
    ...
    useEffect(() => {
        async function fetchData() {
            const t: Torrent = await fetchApi(`torrents/${slug}`);
            setTorrent(t)
        }
    ...
    const handleDownload = useCallback(async (torrent: Torrent) => {
        ...
        link.href = window.URL.createObjectURL(await fetchApi(`torrents/download/${torrent.slug}`, {}, 'blob'));
    ...
    return (torrent && (<>
        ...
        <Container>
            ...
            <Information title="Upload"
                         description={`${format(torrent?.createdAt, 'dd/MM/yyyy')} (${formatDistance(new Date(torrent?.createdAt), new Date(), {addSuffix: true})})`}/>
            ...
            <Information
                ...
                description={
                    <>
                        {torrent.languages.map((language: Language) => (
